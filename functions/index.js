...
export const createWallet = functions.
  https.onCall((data, context) => {
    ...
    if (!context.auth) {
      throw new HttpsError("authentication-error", "Authetication required!");
    }

    // // Connect to the lnd server and activate lightining (if needed)
    // await activateLightning();
 ...
 */
async function activateLightning() {
  if (grpc == null) {
  ...
  if (grpc.state != "active") {
    await grpc.connect();
    if (grpc.state == "ready") {
      throw new HttpsError("server-offline", "Can't connect to the LND server");
    }
  }
